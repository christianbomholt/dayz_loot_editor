...
from model.item import Base, Item, LinkBullets, LinkAttachments, LinkMags, Attachments, Magazines, Bullets
        ...
        for weapon in weapons:
            if weapon.nominal > 0:
                ...
                if tagattach is not None:
                    write_subelement(tagtype, "attachments",
                                     "hndgrd", 0.75, tagattach)
                    write_subelement(tagtype, "attachments",
                                     "bttstck", 0.75, tagattach)
                    write_subelement(tagtype, "attachments",
                                     "optic", 0.25, tagattach)
                    write_subelement(tagtype, "attachments",
                                     "comp", 0.25, tagattach)
                    write_subelement(tagtype, "attachments",
                                     "suppressor", 0.25, tagattach)
                    write_subelement(tagtype, "attachments",
                                     "light", 0.25, tagattach)
...
def write_subelement(root, tag, grp, prop, item_list):
    ...
    if check is True:
        ...
        for item in item_list:
            if grp in item.name.lower():
                ET.SubElement(attach_sub, "item", name=item.name,
                              chance=str(item.prop/100))
