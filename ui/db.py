...
class DB(object):
    def __init__(self, root):
        ...
        self.DbInit = OptionMenu(button_frame, self.DbInitValue, *optionList).grid(
        ...
        # Buttons
        Button(button_frame, text="Open DB", width=12, command=self.openDB).grid(
            row=6, column=0, sticky="w", padx=5
        )
        Button(button_frame, text="New DB", width=12, command=self.newDB).grid(
