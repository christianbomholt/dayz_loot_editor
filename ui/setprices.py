...
class TraderEditor(object):
    ...
    def __create_sub_types(self):
        ...
        traders_option_menu = OptionMenu(
            subtypes_frame,
            self.selected_trader,
        ...
        self.serchName = Entry(
            subtypes_frame, textvariable=self.searchName, width=14
    ...
    def __create_trader_setting(self, root, row, column):
        ...
        self.v.set("rar")
        Radiobutton(
            radio_frame, text=modes[0][0], variable=self.v, value=modes[0][1]
        ).grid(row=0, column=0)
        Radiobutton(
            radio_frame, text=modes[1][0], variable=self.v, value=modes[1][1]
        ).grid(row=0, column=1)
        frame = Frame(root)
        ...
        Button(button_frame, text="Distribute", command=self.__distribute_pricing).grid(
            row=0, column=0
        )
        Button(button_frame, text="Fractions", command=self.__apply_fractions).grid(
            row=0, column=1
        )
        Button(button_frame, text="Stock", command=self.__apply_stock).grid(
            row=0, column=2
        )
        Button(button_frame, text="Save Changes", command=self.update).grid(
            row=0, column=3
        )
        Button(button_frame, text="DrJones Trader", command=self.__create_trader).grid(
            row=0, column=4, padx=5
        )
        Button(
            button_frame, text="Expansion Trader", command=self.__create_expansiontrader
    ...
    def __fill_trader_window(self, root):
        ...
        selected_sub_type = (
            "" if selected_sub_type == "UNASSIGNED" else selected_sub_type
        )
    ...
    def __apply_fractions(self):
        ...
        for item in self.traderVal:
            sell_price = (
                int(float(item[0][1].get()) * float(self.fraction.get()))
                if item[0][1].get() != "-1"
                else -1
            )
    ...
    def __distribute_pricing(self):
        ...
        try:
            pricing = distribute(
                rarities,
                int(self.buy_entries[0].get()),
                int(self.buy_entries[1].get()),
                int(self.sellEntries[0].get()),
                int(self.sellEntries[1].get()),
                rarity_is_set,
            )
        except IndexError:
            messagebox(
