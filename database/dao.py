...
class Dao(object):
    ...
    def upgradeDB(self):
        try:
            sql_String = (
                f'{"ALTER TABLE items ADD COLUMN min_Stock INTEGER DEFAULT 10"}'
            )
        ...
        try:
            sql_String = (
                f'{"ALTER TABLE items ADD COLUMN max_Stock INTEGER DEFAULT 100"}'
            )
            self.session.execute(sql_String)
            self.session.commit()
    ...
    def get_filter_types(self, cat):
        ...
        result = (
            ...
            .filter(Item.cat_type.like(search))
            .all()
        )
    ...
    def get_filter_subtypes(self, types):
        ...
        result = (
            self.session.query(Item)
            .distinct(Item.sub_type)
            .filter(Item.item_type.like(search))
            .all()
        )
        row_list = [c.sub_type for c in result if c.sub_type is not None]
        result = list(sorted(set(row_list)))
        result.append("all")
        return result
    ...
    def getNominal(self, grid_items):
        ...
        result = (
            self.session.query(func.sum(Item.nominal))
    ...
    def getNominalByCat(self, grid_items, cat_type):
        grid_items = grid_items.subquery()
        result = (
            self.session.query(Item.cat_type, func.sum(Item.nominal))
    ...
    def getNominalByType(self, grid_items, item_type):
        grid_items = grid_items.subquery()
        result = (
            self.session.query(Item.item_type, func.sum(Item.nominal))
    ...
    def search_subtypeby_name(self, item_name):
        search = f"%{item_name}%"
        results = (
            self.session.query(Item.name, Item.sub_type, Item.trader)
    ...
    def search_attach_name(self, attachClasse, item_name):
        search = f"%{item_name}%"
        results = (
            self.session.query(attachClasse)
            .filter(attachClasse.name.like(search))
            .all()
        )
        return results
    ...
    def get_tradersubtypetupl(self, traderSel, selected_Mods):
        results = (
            self.session.query(Item.sub_type)
            .filter(and_(Item.trader == (traderSel), Item.mod.in_(selected_Mods)))
            .group_by(Item.sub_type)
            .order_by(Item.sub_type)
            .all()
        )
    ...
    def setTraderValues_fast(self, values):
        for value in values:
            self.session.query(Item).filter(Item.name == value[7]).update(
                {
    ...
    def setSubtypeForTrader_fast(
        self, traderCat, buyprice, sellprice, traderExclude, rarity, name
    ):
        self.session.query(Item).filter(Item.name.in_(name)).update(
            {
    ...
    def setmapselectValue(self, value):
        ...
        if not exists:
            map_object = Mapselect(mapselectvalue=value)
